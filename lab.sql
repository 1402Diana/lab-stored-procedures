...
DROP PROCEDURE IF EXISTS GetMovieCustomersByCategory;
DELIMITER //
CREATE PROCEDURE GetMovieCustomersByCategory(IN category_name VARCHAR(50))
BEGIN
    SELECT first_name, last_name, email
    FROM customer
    JOIN rental ON customer.customer_id = rental.customer_id
...
DROP PROCEDURE IF EXISTS GetCategoriesWithMinMovies;
DELIMITER //
CREATE PROCEDURE GetCategoriesWithMinMovies(IN min_movie_count INT)
BEGIN
    SELECT category.name, COUNT(film.film_id) AS movie_count
    FROM category
    JOIN film_category ON category.category_id = film_category.category_id
